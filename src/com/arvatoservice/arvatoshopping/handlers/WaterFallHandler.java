package com.arvatoservice.arvatoshopping.handlers;
...
import com.arvatoservice.arvatoshopping.view.WaterFallView;
...
public class WaterFallHandler extends Handler {
	...
	public void handleMessage(Message msg)
	{
		...
		{
		case HANDLER_WHAT:
			if (mColumeView.getMeasuredHeight() - mSensitivityY <= waterFallView
					.getScrollY() + waterFallView.getHeight())
			{
				if (BuildConfig.DEBUG)
				{
					Log.d(WaterFallView.TAG,
							"onBottom"
									+ "childViewHeight->"
									+ mColumeView.getMeasuredHeight()
									+ "waterFallView->"
									+ (waterFallView.getScrollY() + waterFallView
											.getHeight()));
				}
			...
			else if (waterFallView.getScrollY() <= 0)
			{
				if (waterFallView.onScrollListener != null)
				{
					...
					{
						Log.d(WaterFallView.TAG,
								"onTop->" + waterFallView.getScrollY());
					}
