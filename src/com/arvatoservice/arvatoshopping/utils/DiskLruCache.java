...
public class DiskLruCache {
        ...
        synchronized (mLinkedHashMap) {
            if (mLinkedHashMap.get(key) == null) {
                try {
                	...
                	CacheUtils.Debug("Error in put: " + e.getMessage());
    ...
    private void flushCache() {
        ...
        while (count < MAX_REMOVALS &&
            ...
            if (BuildConfig.DEBUG) {
                CacheUtils.Debug("flushCache - Removed cache file, " + eldestFile + ", "
                        + eldestFileSize);
            }
        }
    }
    ...
    public Bitmap get(String key) {
        synchronized (mLinkedHashMap) {
            ...
            if (file != null) {
                	CacheUtils.Debug("Disk cache hit");
                return BitmapFactory.decodeFile(file);
            } else {
                final String existingFile = createFilePath(mCacheDir, key);
                if (new File(existingFile).exists()) {
                    put(key, existingFile);
                    CacheUtils.Debug("Disk cache hit (existing file)");
...
//        Log.e("getDiskCacheDir", cachePath + File.separator + uniqueName+"");
    ...
    public static String createFilePath(File cacheDir, String key) {
        try {
            ...
            CacheUtils.Debug( "createFilePath - " + e);
